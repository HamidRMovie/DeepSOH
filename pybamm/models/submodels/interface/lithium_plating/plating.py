...
class Plating(BasePlating):
    ...
    **Extends:** :class:`pybamm.lithium_plating.BasePlating`
    ...
    def get_coupled_variables(self, variables):
        ...
        j_tot_av = variables["X-averaged negative electrode total interfacial current density"]
        ##

        #Hamid_August16_2026
        w_current_area = pybamm.Parameter(
            "Use current-area plating thickness"
        )
        c_plated_Li_dim = variables["Lithium plating concentration [mol.m-3]"]
        a_s_dim = variables[
            "Negative electrode surface area to volume ratio [m-1]"
        ]
        delta_pl_current = (
            param.V_bar_plated_Li * c_plated_Li_dim / a_s_dim
        )
        delta_pl_current_av = pybamm.x_average(delta_pl_current)

        delta_pl_legacy = variables["Lithium plating thickness [m]"]
        delta_pl_legacy_av = variables["X-averaged lithium plating thickness [m]"]
        delta_pl_effective = (
            (1 - w_current_area) * delta_pl_legacy
            + w_current_area * delta_pl_current
        )
        delta_pl_effective_av = (
            (1 - w_current_area) * delta_pl_legacy_av
            + w_current_area * delta_pl_current_av
        )

        variables.update(
            {
                "Current-area lithium plating thickness [m]": delta_pl_current,
                "X-averaged current-area lithium plating thickness [m]": delta_pl_current_av,
                "Effective lithium plating thickness [m]": delta_pl_effective,
                "X-averaged effective lithium plating thickness [m]": delta_pl_effective_av,
            }
        )
        ...
        # eta_stripping = delta_phi + phi_ref + eta_sei + eta_plated_Li  - U_pl
